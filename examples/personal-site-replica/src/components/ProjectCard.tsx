...
export function ProjectCard({ project, eager = false }: { project: Project; eager?: boolean }) {
  const optimized = project.featured ? imageMap[project.slug] : undefined
  return (
    <article className="project-card" data-project-card data-project-tags={project.tags.map((tag) => tag.toLowerCase()).join(',')}>
