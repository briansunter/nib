...
url: "https://briansunter.com/why-clojure"
---
...
![Cover image for Why Clojure?](/_astro/clojure_logo.svg_1660983015847_0.mGDJpw4G_11BUfa.webp)
...
- **S-Expressions** (parentheses) are reusable and composable
- **Interactive programming** with a high-speed feedback loop via the REPL
- **Powerful testing** due to emphasis on pure functions and values
- **Good interop** with Java and JavaScript
- **Excellent concurrency support**: immutability, software transactional memory, core.async channels, agents
- **Good design** with strong notions of time, identity, state, and equality
...
In most languages, the `reverse` function changes the original array. Everyone who references the original array sees the change.
...
The same test in Clojure is more concise:

clojure

```
(deftest test-backstage-pass-peak
  (let [max-quality-pass {:name BACKSTAGE_PASS_ITEM_NAME :quality 50 :sell-in 5}]
...
I feel bad about using languages like Python or Javascript that are inherently single-threaded. Languages like Java or C++, which weren’t designed with concurrency, are hard to use correctly.

Clojure’s data structures are thread-safe by default, and it has numerous concurrency primitives. The language design de-emphasizes the use of state and emphasizes using values instead, which makes concurrency much simpler.

## I must have types

Initially, I was hesitant about Clojure because I was used to statically typed languages like Java and C++. If you rely on types, consider the tradeoffs: the coupling introduced by type information flowing through your program, versus the safety guarantees they provide. Clojure also has optional type checking via [core.spec](https://clojure.org/guides/spec) for runtime validation.
...
if (a == b) {
    System.out.println("True");   // Prints "False" - compares references
} else {
    System.out.println("False");
}

if (a.equals(b)) {
    System.out.println("True");   // Prints "True" - compares values
} else {
    System.out.println("False");
}
```
...
(defmethod area :default [_] :error)
...
[Newer emacs-daemon-mode-on-macos](/emacs-daemon-mode-on-macos)
