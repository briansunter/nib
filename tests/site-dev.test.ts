...
describe('framework-owned development server', () => {
  it('serves consumer routes below the configured base path', async () => {
    ...
    expect(homeHtml).not.toContain('data-nib-behaviors')
    expect(enhanced.status).toBe(200)
    const enhancedHtml = await enhanced.text()
    expect(enhancedHtml).toContain('data-nib-behavior="reveal"')
    expect(enhancedHtml).toContain('data-nib-behaviors')
    expect(enhancedHtml).toContain('src="/journal/@id/virtual:nib/behavior-entry"')
