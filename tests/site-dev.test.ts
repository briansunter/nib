...
describe('framework-owned development server', () => {
  it('serves consumer routes below the configured base path', async () => {
    ...
    const enhanced = await request('/journal/enhanced/')
    ...
    expect(homeHtml).not.toContain('data-nib-enhancements')
    expect(enhanced.status).toBe(200)
    const enhancedHtml = await enhanced.text()
    expect(enhancedHtml).toContain('data-nib-enhancement="reveal"')
    expect(enhancedHtml).toContain('data-nib-enhancements')
    expect(enhancedHtml).toContain('src="/journal/@id/virtual:nib/enhancement-entry"')
